...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _imageUploadLinker = __webpack_require__(13);

	var _imageCache = __webpack_require__(14);

	var _persistance = __webpack_require__(15);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _paragraph = __webpack_require__(6);

	var _table = __webpack_require__(9);

	var _inlineImage = __webpack_require__(10);
	...
	function groupListItems(processedElements, el, idx, elements) {
		var type = el.getType();
		...
		var list = previousIsListItem ? new ListElement() : (0, _lodash.tail)(processedElements);
		list.addListItem(el);

		return previousIsListItem ? processedElements : [].concat(_toConsumableArray(processedElements), [list]);
	}
	...
	function DocService(DocumentApp, imageLinker) {
		...
		var renderContainer = function renderContainer(element) {
			return childrenOf(element).reduce(groupListItems).map(renderElement).concat(renderPositionedImages(element)).map(removeAutolinkedUrls).join('');
		};
...
/***/ (function(module, exports) {
	...
	function List(DocumentApp, renderContainer) {
		var renderListItem = ListItem(DocumentApp, renderContainer);
		...
		return function renderList(list) {
			var openTag = '<' + tagForList(list.items[0]) + typeForList(list.items[0]) + '>';
			var body = list.items.map(function (li) {
				return li.getType === 'A8C.List' ? renderList(li) : renderListItem(li);
			});
