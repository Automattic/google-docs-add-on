	...
	function DocService(DocumentApp, imageLinker) {
	  ...
	  */
		var renderContainer = function renderContainer(element) {
			var groupedChildren = groupListItems(childrenOf(element));
			return groupedChildren.map(renderElement).concat(renderPositionedImages(element)).map(removeAutolinkedUrls).join('');
		};
